...
void unfold ()
{
	struct h *h;
	/* PAOLO */
	int ret;  /* return value for marking_add: was a weak causal
		     place found? */
	int bndc; /* is the net BNDC ? */


	bndc = 1;   /* unless something will go wrong, the net is BNDC */
	...
	printf("--------------------------------------------------------\n");
	if (bndc) 
	  printf("The net is BINI.\n");
	else
	  printf("The net is not BINI.\n");
