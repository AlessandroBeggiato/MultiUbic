...
struct place {
	...
	int causal;     /* ALESSANDRO: flag to mark weack/intransitive causal places */
};
...
struct trans {
	...
	/*
	 * ALESSANDRO: added a field to store the security level
	 * dummy transition t0 will be assigned the special level -1
	 */
	int level;
};
...
struct cond {
	...
	struct nl* absorbs; /* ALESSANDRO: list of conditions absorbed (downgraded) by this */
	int id;			/* internal numeric identifier */
	int m;			/* general purpose mark */
	int cnt;		/* see ec_conc */
	//ALESSANDRO: moved this field to struct place
	//int is_weak_causal;     /* PAOLO: is it a weak causal place? */
};
...
struct u {
	...
	/* PAOLO: is the prefix required to be complete or stopped at first weak causal place? */
	int complete;
	/* int interactive; */
	/* ALESSANDRO: maximum number of histories */
	int historyLimit;
	struct bv pmask;
};
...
// FRANCESCO
/*
 * Hash table to memorize the indexes of the high variables.
 */
struct high_hash {
  const char * hname;          /* key */
  int index;
  UT_hash_handle hh;           /* makes this structure hashable */
};

struct high_hash * high_hash;
// \FRANCESCO
struct u u;
