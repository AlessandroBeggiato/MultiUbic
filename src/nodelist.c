...
{
	while (*list && cmp (node, (*list)->node) > 0)
		list = &((*list)->next);
...
{
	while (list1 && list2 && cmp(list1->node, list2->node) == 0)
		list1 = list1->next, list2 = list2->next;

	if (!list1 && !list2) return 0;
	if (!list1) return -1;
	if (!list2) return 1;
	return cmp(list1->node, list2->node);
}

/* ALESSANDRO********************************************/
/* test if an element appears in an ordered list, returns 1 if so, 0 otherwise */
int nl_test_ord (const struct nl *list, void *node)
{
...
/* PAOLO**********************************************************************/
/* check if an element appears in a list; return 1 if so, 0 otherwise    */

int nl_test (const struct nl *list, void *node)
{
  while (list &&  (node != list->node))
    list = list->next;
...
char nl_find_custom(struct nl * list, void * element,
  ...
  while (list && cmp(list->node,element))
      list = list->next;
    if (list)
      return 1;
    else
...
/*
 * As nl_delete but it also releases the memory of list->node
 */
